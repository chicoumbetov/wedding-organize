...
export const getCommentThunk = createAsyncThunk("posts/comments", async () => {
  const { data } = await api.commentList();
  return data;
});

export const getUserLikesThunk = createAsyncThunk("users/likes", async () => {
  const { data } = await api.likesList();
  return data;
});

// createAsyncThunk creates Action automatically
export const getUsersThunk = createAsyncThunk("users/fetchUsers", async () => {
  try {
